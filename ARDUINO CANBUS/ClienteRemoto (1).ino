...
*/
    ...
    CAN.attachInterrupt(0, MCP2515_ISR, FALLING); // start interrupt
}
...
void loop()
{
  ...
  CAN.sendMsgBuf(DIR_OBDII, 0, 3, stmp); 
    if(flagRecv) 
    ...
    Serial.print(";");
    if (obdPID==PID_2){
    Serial.print(float(buf_Recv[3])/200, DEC);
    Serial.println(";");
    Serial.println((buf_Recv[4]/1.28)-100, DEC);
    }
    if (obdPID==PID_1){
       Serial.print(buf_Recv[3]*3, DEC);
       Serial.println(";");
    }
    Serial.println("Fin de Peticion");
    }
  delay(1000);
    }
